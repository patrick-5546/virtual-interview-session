...
int * read_image(void)
{
   ...
   for (i = 0; i < 50176; ++i) {
      *IMAGE_READER_ptr = i; // write the desired pixel to component
      x = i % 224;
      y = i / 224;
      // first 8x8 block has DCT, the rest has Y channel
      y_channel = *IMAGE_READER_ptr & 0xFFFF;
      dct = (*IMAGE_READER_ptr >> 16) & 0xFFFF;
      if (x < 8 && y < 8)
         printf("\tPixel %d: %x, %x\n", i, y_channel, dct);
      img[i] = (x < 8 && y < 8) ? (*IMAGE_READER_ptr >> 16) & 0xFFFF : *IMAGE_READER_ptr & 0xFFFF;
   }
...
int main(void)
{
   ...
   printf("\nPrinting the first 10 elements of img\n");
   int * img = read_image();
   int i, y_channel, dct;
   for (i = 0; i < 10; ++i) {
      printf("\tPixel %d: %x\n", i, img[i]);
